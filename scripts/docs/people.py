...
def get_contributors_gh_cli(repo_name: str) -> list[dict[str, Any]]:
    ...
    try:
        result = subprocess.run(
            ["gh", "api", f"repos/{repo_name}/contributors", "--paginate"], capture_output=True, text=True, check=True
        )
...
def format_contributors(contributors: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for c in contributors:
        formatted.append(
            {"login": c["login"], "count": c["contributions"], "avatarUrl": c["avatar_url"], "url": c["html_url"]}
        )
