...
def log_config():
    ...
    warnings.warn(
        ...
        "from apple_reminders_mcp.monitoring.logging for thread-safe logging in production.",
        DeprecationWarning,
        stacklevel=2,
    )
...
def setup_thread_safe_logging(
    ...
    log_file_path: str = "logs/apple_reminders_mcp.log",
    ...
    Example:
        from apple_reminders_mcp.monitoring.logging import early_init, setup_thread_safe_logging
...
def setup_apple_reminders_mcp_logging(settings=None) -> "Logger":
    """
    ...
    configures logging based on McpSettings and environment.

    Args:
        settings: Optional McpSettings instance. If not provided, will try to import and create one.

    Returns:
        Configured logger instance

    Example:
        from apple_reminders_mcp.monitoring.logging import early_init, setup_apple_reminders_mcp_logging
        from apple_reminders_mcp.core.env import McpSettings
        ...
        logger = setup_apple_reminders_mcp_logging()
    """
    # Ensure early init was called
    if not is_early_init_done():
        early_init()
    ...
    if settings is None:
        try:
            ...
            # Fall back to default configuration if McpSettings not available
...
__all__ = [
    ...
    "setup_apple_reminders_mcp_logging",  # Boss-bot specific setup
    "configure_thread_safe_logging",
    "ThreadSafeLogConfig",
