...
def parse_args() -> argparse.Namespace:
    ...
    """
    parser = argparse.ArgumentParser(description="Serve MkDocs documentation with automatic port management")
    parser.add_argument("--port", type=int, default=8000, help="Port to use for the server (default: 8000)")
    parser.add_argument("--kill-existing", action="store_true", help="Kill existing MkDocs processes using the port")
    parser.add_argument(
        "--no-kill-existing",
        action="store_false",
        dest="kill_existing",
    ...
    parser.add_argument("--build-only", action="store_true", help="Only build the documentation, don't serve it")
...
def serve_docs(default_port: int = 8000, kill_existing: bool = False, gh_deploy_url: bool = False) -> int:
    """
    ...
    try:
        if args.build_only:
            ...
            exit_code = serve_docs(
                default_port=args.port, kill_existing=args.kill_existing, gh_deploy_url=args.gh_deploy_url
            )
            return exit_code
    finally:
