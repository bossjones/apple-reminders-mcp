...
def check_yaml_header(file_path: str) -> tuple[bool, list[str], dict[str, Any]]:
    ...
    """
    issues = []
    frontmatter = {"description": "", "globs": "", "alwaysApply": False, "rule_type": "Unknown"}
    ...
    try:
        ...
        rule_type, type_issues = determine_rule_type(
            frontmatter["description"], frontmatter["globs"], frontmatter["alwaysApply"]
        )
...
def print_rule_type_examples(console: Console) -> None:
    ...
    rule_types = [
        ...
        (
            "Always",
            "magenta",
            "Applied to every chat and cmd-k request automatically. No need for description or globs.",
        ),
    ...
    for rule_type, color, description in rule_types:
        ...
        if rule_type == "Agent Selected":
            ...
            example_text = "---\ndescription:\nglobs:\nalwaysApply: true\n---"
            field_notes = (
                "description: [bold red]BLANK[/bold red]\nglobs: [bold red]BLANK[/bold red]\nalwaysApply: must be true"
            )
        ...
        panel = Panel(panel_content, title=f"[bold]{rule_type}[/bold]", border_style=color, padding=(1, 2))
        console.print(panel)
...
def parse_arguments() -> argparse.Namespace:
    ...
    Returns:
        Parsed arguments
    """
    parser = argparse.ArgumentParser(description="Audit cursor rule files for proper YAML frontmatter headers.")
    parser.add_argument(
        "--prod",
        action="store_true",
        help="Check production directory (.cursor/rules) instead of staging (hack/drafts/cursor_rules)",
    )
    parser.add_argument("--desc", action="store_true", help="Include the description field in the output table")
...
def main() -> None:
    ...
    for file_path, frontmatter in frontmatter_info.items():
        ...
        # Prepare row data
        row_data = [relative_path, f"[{color}]{rule_type}[/{color}]", glob_patterns]
