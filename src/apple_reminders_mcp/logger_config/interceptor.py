...
Usage:
    ...
    from apple_reminders_mcp.monitoring.logging.interceptor import early_init
    early_init()
    ...
    from apple_reminders_mcp.monitoring.logging.interceptor import configure_thread_safe_logging
...
def early_init() -> None:
    ...
    with _early_init_lock:
        ...
        critical_loggers = [
            ...
            "apple_reminders_mcp",
            "apple_reminders_mcp.bot",
            "apple_reminders_mcp.bot.client",
            "apple_reminders_mcp.bot.cogs",
            "apple_reminders_mcp.core",
            "apple_reminders_mcp.core.downloads",
            "apple_reminders_mcp.core.queue",
            "apple_reminders_mcp.cli",
            "apple_reminders_mcp.monitoring",
            "apple_reminders_mcp.storage",
            "apple_reminders_mcp.utils",
...
class ThreadSafeLogConfig(BaseModel):
    ...
    Can be configured from McpSettings for full integration.
    """

    log_level: str = "INFO"
    enable_file_logging: bool = False  # Default to stdout only
    log_file_path: str = "logs/apple_reminders_mcp.log"  # Used only if file logging enabled
    ...
    def from_boss_settings(cls, settings: Any) -> ThreadSafeLogConfig:
        """Create ThreadSafeLogConfig from McpSettings.

        Args:
            settings: Boss-bot settings instance (McpSettings)
        ...
        log_file_path = str(settings.storage_root / "logs" / "apple_reminders_mcp.log")
