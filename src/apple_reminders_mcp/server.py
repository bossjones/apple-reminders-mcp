...
async def create_reminder(
    ...
    notes: str | None = None,
    list_name: str | None = "Reminder Created using Agent",
    ...
    try:
        ...
        script_part.extend([f'set targetList to list "{list_name}"', "move newReminder to targetList", "end tell"])

        script = "\n".join(script_part)
        result = run_applescript(script)
        return f"Reminder '{title}' created successfully in list '{list_name}'. AppleScript Output: {result}"
    except Exception as e:
        return f"Failed to create reminder: {e!s}"


@mcp.tool()
async def get_reminder(list_name: str | None = None, completed: bool = False, limit: int = 20) -> str:
    """
...
'''
        ...
        return json.dumps({"error": f"Failed to get reminders: {e!s}"}, indent=2)
...
"""
        ...
        return json.dumps({"error": f"Failed to get reminder lists: {e!s}"}, indent=2)


@mcp.tool()
async def delete_reminder(name: str, list_name: str | None = None) -> str:
    """
...
'''
        ...
        return f"Failed to delete reminder: {e!s}"
