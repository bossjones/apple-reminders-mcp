...
COMMIT_PATTERN = re.compile(r"^(?P<type>[a-z]+)(?:\((?P<scope>[a-z0-9/-]+)\))?: (?P<subject>.+)$", re.IGNORECASE)
BREAKING_PATTERN = re.compile(r"BREAKING CHANGE:", re.IGNORECASE)

# Regex for parsing the changelog
VERSION_PATTERN = re.compile(r"## \[(?P<version>[^\]]+)\](?: - (?P<date>\d{4}-\d{2}-\d{2}))?")
...
def parse_commit(commit: str, config: ChangelogConfig) -> tuple[str, str, bool] | None:
    """
...
def main() -> None:
    ...
    parser.add_argument("--since", help="Get commits since date (e.g., '2023-01-01')")
    parser.add_argument("--until", help="Get commits until date (e.g., '2023-12-31')")
    parser.add_argument(
        "--types",
