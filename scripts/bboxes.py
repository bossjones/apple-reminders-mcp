...
def gemini_retry(
    max_retries: int | None = None, min_wait: float | None = None, max_wait: float | None = None
...
@gemini_retry()
def generate_gemini_content(model: genai.GenerativeModel, prompt_parts: list[dict[str, str | bytes] | str]) -> Any:
    """
...
def resize_image_with_background(image: PIL.Image.Image, output_path: str) -> None:
    """
...
Examples:
    ...
    parser.add_argument(
        "--image-path", type=str, required=False, help="Path to the input image file or directory containing images"
    )
    parser.add_argument(
        "--output-path", type=str, help="Path to save the output image or directory for multiple images"
    )
    parser.add_argument(
        "--mode",
        type=str,
        choices=["tweet", "general"],
        default="tweet",
        help="Detection mode: 'tweet' for tweet content or 'general' for all objects",
    )
    parser.add_argument("--box-color", type=str, default="red", help="Color of the bounding box (name or hex code)")
    parser.add_argument("--box-width", type=int, default=4, help="Width of the bounding box line")
    parser.add_argument("--label", type=str, help="Custom label for the bounding box (tweet mode only)")
    parser.add_argument(
        "--autocrop", action="store_true", help="Crop image to the detected area instead of drawing a bounding box"
    )
    parser.add_argument(
        "--crop-percent",
        type=float,
    ...
    parser.add_argument(
        "--temperature",
        type=float,
        default=settings.GEMINI_TEMPERATURE,
...
def detect_tweet_content(
    ...
    try:
        ...
        logger.debug(
            f"Using box color: {box_color}, width: {box_width}, label: {label}, autocrop: {autocrop}, crop_percent: {crop_percent}, resize: {resize}, temperature: {temperature}"
        )
        ...
        if all(key in tweet_box for key in required_keys):
            ...
            if all(isinstance(coord, (int, float)) for coord in [xmin, ymin, xmax, ymax]):
                ...
                logger.debug(
                    f"Converted to absolute coordinates: xmin={abs_xmin}, ymin={abs_ymin}, xmax={abs_xmax}, ymax={abs_ymax}"
                )
                ...
                if autocrop:
                    ...
                    logger.debug(
                        f"Original height: {abs_ymax - abs_ymin}px, Cropped height: {padded_ymax - padded_ymin}px, Crop percent: {crop_percent}%"
                    )
                    logger.debug(
                        f"Removed approximately {abs_ymax - timestamp_position}px from bottom to exclude timestamp/views"
                    )
                else:
                ...
                logger.debug(
                    f"Added padding to coordinates: xmin={padded_xmin}, ymin={padded_ymin}, xmax={padded_xmax}, ymax={padded_ymax}"
                )
                ...
                if autocrop:
                    ...
                    draw.rectangle(
                        [(padded_xmin, padded_ymin), (padded_xmax, padded_ymax)], outline=box_color, width=box_width
                    )
...
def detect_objects_and_draw_boxes(
    ...
    try:
        ...
        logger.debug(
            f"Using box color: {box_color}, width: {box_width}, autocrop: {autocrop}, resize: {resize}, temperature: {temperature}"
        )
        ...
        if isinstance(object_data, list):
            ...
            for i, obj in enumerate(object_data):
                ...
                logger.debug(
                    f"Processing object: {label} at coordinates: xmin={xmin}, ymin={ymin}, xmax={xmax}, ymax={ymax}"
                )

                if all(
                    isinstance(coord, (int, float)) for coord in [xmin, ymin, xmax, ymax]
                    ...
                    if autocrop:
                        ...
                        object_output = os.path.join(
                            output_dir, f"{output_name}_{i + 1}_{label.lower().replace(' ', '_')}{output_ext}"
                        )
                        ...
                        draw.text(
                            (padded_xmin, padded_ymin - 10), label, fill=box_color
