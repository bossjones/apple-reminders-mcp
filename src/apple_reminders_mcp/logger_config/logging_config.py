...
class LogConfig(BaseModel):
    ...
    use the thread-safe logging from apple_reminders_mcp.monitoring.logging instead.
    """

    LOGGER_NAME: str = "apple_reminders_mcp"
    LOG_FORMAT: str = "<green>{time:YYYY-MM-DD HH:mm:ss.SSS}</green> | <level>{level: <8}</level> | <cyan>{name}</cyan>:<cyan>{function}</cyan>:<cyan>{line}</cyan> - <level>{message}</level>"
    LOG_LEVEL: str = "INFO"
    LOG_FILE_PATH: Path = Path("logs/apple_reminders_mcp.log")
    ENABLE_FILE_LOGGING: bool = False  # Default to stdout only for containerized environments

    @classmethod
    def from_boss_settings(cls, settings: "McpSettings") -> "LogConfig":
        """Create LogConfig from McpSettings.
        ...
        log_file_path = settings.storage_root / "logs" / "apple_reminders_mcp.log"
    ...
    def setup_logging(self, settings: Optional["McpSettings"] = None) -> None:
